...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('✪ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/tu_telugu_bacup"),
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAEBrw5k3MZc8jmzlvQcqsXasrByhhPMAAMoCgACSmvpVh8mMhfwCi0WHgQ") 
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          ...
                          InlineKeyboardButton("Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url="https://t.me/+3iUhYfWqMMk4MGU1")
                         ]
                        ]
                    )
                )
    ...
    if not files_:
        ...
        try:
            ...
            title = '@tu_telugu_bacup ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@tu_telugu_bacup ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
                    ...
                    InlineKeyboardButton('ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ', url=f'https://t.me/tu_url/26')
                ]]
        await message.reply_text(
            text="<b>You are not verified !\nKindly verify to continue ! So That you Can Get Access To Unlimited Movies Until 12 Hours From Now ! </b>",
            protect_content=True,
        ...
        reply_markup=InlineKeyboardMarkup(
            [
             [
              ...
              InlineKeyboardButton("Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ", url="https://t.me/+WRV57Pro2U02N2E9")
             ]
            ]
        )
    )
